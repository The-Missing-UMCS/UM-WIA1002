...
public class Main {
    ...
    private static void runComparison(int initialSize, int finalSize,
        ...
        for (int size = initialSize, i = 0; size <= finalSize; size *= 10, i++) {
            ...
            System.out.printf("When size = %d, total time used = %s%n", size,
                Arrays.toString(totalTimes));
     ...
     * recursive binary. Each method is tested repeatedly with newly generated arrays and random
     * targets.
     *
    ...
    private static long[] measureSearchTime(int size, int low, int high, int repeat) {
        ...
        for (int i = 0; i < repeat; i++) {
            ...
            totalTimes[2] += measure(
                () -> Searching.binarySearchRecursive(arr, 0, arr.length - 1, target));
        }
